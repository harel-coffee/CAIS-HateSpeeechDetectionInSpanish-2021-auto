...
def main ():
    ...
    choices = FeatureResolver.get_feature_combinations (['lf', 'se', 'be', 'we', 'ne', 'cf', 'bf', 'pr', 'ng', 'it'])
    ...
    def callback (feature_key, y_pred, model_metadata):
        ...
        if ('probabilities' in model_metadata) and model_metadata['probabilities'] is not None and (args.model not in ['machine-learning', 'ensemble', 'transformers']) and ('classification' == task_type):
