...
class DeepLearningTechniques (BaseModel):
    ...
    all_available_features = ['lf', 'se', 'be', 'we', 'ne', 'cf', 'bf', 'pr', 'ng', 'it']
    ...
    def train (self, force = False, using_official_test = True):
        ...
        if self.dataset.dataset == 'satire' and self.dataset.corpus == 'full':
            params_batch_size = [256, 512]

        if self.dataset.dataset == 'collaborativehealth':
            params_batch_size = [256, 512]

        # Due to the size level, we reduce the batch size
        if self.dataset.dataset == 'policorpus' and 'user_level' in self.dataset.task:
            params_batch_size = [4, 8, 16]
            # params_lr = [10e-03, 10e-04, 10e-05]
            params_lr = [10e-03]

        if self.dataset.dataset == 'policorpus' and 'tweet_level' in self.dataset.task:
            params_batch_size = [128, 256, 512]
        ...
        if self.dataset.dataset in ['satire', 'collaborativehealth']:
            architectures_to_evaluate = ['lstm', 'gru'] if 'we' in feature_key else ['dense']
        ...
        if 'dense' in architectures_to_evaluate:
            parameters_to_evaluate.append ({
                ...
                'pretrained_embeddings': params_pretrained_embeddings,
                'lr': [10e-03] if self.dataset.dataset == 'policorpus' and 'user_level' in self.dataset.task else [10e-03, 10e-04]
            })
