...
class MultilabelTrainer (transformers.Trainer):
    """
    @link https://huggingface.co/transformers/main_classes/trainer.html
    """
...
class BertModel (BaseModel):
    ...
    def get_pretrained_model (self):
        ...
        if self.dataset.get_dataset_language () == 'es':
            return 'dccuchile/bert-base-spanish-wwm-uncased'
    ...
    def createDatasetFromPandas (self, df, task_type = 'classification'):
        ...
        if 'classification' == task_type:
            df['_temp'] = df['label'].astype (str).astype ('category').cat.codes
            ...
            lb.fit ([self.dataset.get_available_labels ()])
            
            
            # Encode the labels and merge to the dataframe
            # Note we use "; " to automatically trim the texts
            df['_temp'] = pd.DataFrame (lb.transform (df['label'].str.split ('; ')), index = df.index, columns = lb.classes_).values.tolist ()
    ...
    def train (self, num_train_epochs = 3.0, using_official_test = True, force = False):
        ...
        # Load the tokenizer
        self.tokenizer = transformers.AutoTokenizer.from_pretrained (self.get_tokenizer_filename ())
